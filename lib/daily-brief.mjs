...
const BRIEF_ALLOW_SOURCE_PATTERNS = [
  /yahoo!?ニュース/i,
  /nhk/i,
  /日本経済新聞|nikkei/i,
  /朝日新聞|asahi/i,
...
const BRIEF_EXCLUDE_SOURCE_PATTERNS = [
...
const BRIEF_ALLOW_HOST_PATTERNS = [
  /(?:^|\.)co\.jp$/i,
  /(?:^|\.)or\.jp$/i,
  /(?:^|\.)ne\.jp$/i,
  /(?:^|\.)go\.jp$/i,
...
const BRIEF_EXCLUDE_HOST_PATTERNS = [
  /news\.google\.com$/i,
  /apnews\.com$/i,
  /cnn\.com$/i,
  /foxnews\.com$/i,
...
export function buildDailyBrief({ currentItems = [], archiveItems = [], generatedAt = null } = {}) {
  ...
  const currentScoredItems = dedupeById(currentItems)
    ...
    .filter((item) => isDomesticJapaneseBriefItem(item))
  ...
  const items = scoredItems
    ...
    .sort((left, right) => publishedTimestamp(left) - publishedTimestamp(right))
    .map((item) => ({
    id: item.id,
    title: item.title,
...
function scoreDailyBriefItem(item, now) {
  ...
  publicImportanceScore += scorePublicImportance(item, combinedText, reasons);
  ...
  const categoryPenalty = scoreBriefPenalty(item, combinedText);
  publicImportanceScore -= categoryPenalty;

  const decayRate = ageHours <= 24 ? 1 : ageHours <= 48 ? 0.5 : ageHours <= 72 ? 0.25 : 0;
  const finalScore = Math.round((importanceScore + publicImportanceScore) * decayRate);
...
function isEligibleForBrief(item) {
  ...
  if (item.publicImportanceScore <= -20) return false;
  if (isDisallowedBriefTopic(item)) return false;
  if (item.ageHours > TIMELINE_WINDOW_HOURS) return false;
...
function scorePublicImportance(item, text, reasons) {
  let score = 0;

  if (item.categories.some((category) => ["politics", "business", "crime", "world", "general"].includes(category))) {
    score += 24;
    reasons.push("時事ニュースとして重要度が高い");
  }

  if (/(地震|津波|大雨|台風|豪雨|洪水|避難|噴火|猛暑|停電|断水|交通障害|運休)/.test(text)) {
    score += 28;
    reasons.push("災害・生活影響の可能性");
  }

  if (/(首相|大統領|政府|国会|法案|予算|選挙|日銀|金利|物価|為替|関税|防衛|外交|首脳会談)/.test(text)) {
    score += 24;
    reasons.push("政治・経済・外交の重要動向");
  }

  if (/(逮捕|送検|起訴|判決|事故|死亡|不明|捜査|不祥事|謝罪|回収|リコール)/.test(text)) {
    score += 18;
    reasons.push("社会・事件の続報性が高い");
  }

  if (/(発表|会見|方針|決定|開始|停止|値上げ|値下げ|補助金|年金|保険料|電気代|ガソリン)/.test(text)) {
    score += 14;
    reasons.push("生活判断に影響しやすい");
  }

  return score;
}

function scoreBriefPenalty(item, text) {
  let penalty = 0;

  if (item.categories.some((category) => ["games", "manga", "adult"].includes(category))) penalty += 48;
  if (item.categories.some((category) => ["sports", "sns", "net-culture", "matome"].includes(category))) penalty += 36;
  if (item.categories.some((category) => ["entertainment", "books"].includes(category))) penalty += 22;

  if (/(ポケモン|pokemon|ゲーム|steam|switch|ps5|漫画|マンガ|アニメ|セール|割引|クーポン|dlsite|fanza|ミーム|炎上|バズ|芸能)/.test(text)) {
    penalty += 26;
  }

  if (!isDomesticJapaneseBriefItem(item)) penalty += 100;

  return penalty;
}

function isDisallowedBriefTopic(item) {
  const text = `${item.title ?? ""} ${item.summary ?? ""} ${item.briefSummary ?? ""}`.toLowerCase();
  if (item.categories.some((category) => ["games", "manga", "adult", "sports", "sns", "net-culture", "matome"].includes(category))) return true;
  if (/(ポケモン|pokemon|ゲーム|steam|switch|ps5|漫画|マンガ|アニメ|セール|割引|クーポン|dlsite|fanza|同人|ミーム|炎上|バズ|アダルト|エロ|コスプレ|グラビア|芸能ゴシップ)/.test(text)) return true;
  return false;
}
...
function isDomesticJapaneseBriefItem(item) {
  const sourceName = briefSourceName(item);
  if (BRIEF_ALLOW_SOURCE_PATTERNS.some((pattern) => pattern.test(sourceName))) return true;
  if (BRIEF_EXCLUDE_SOURCE_PATTERNS.some((pattern) => pattern.test(sourceName))) return false;

  const host = briefSourceHost(item);
  if (host) {
    if (BRIEF_EXCLUDE_HOST_PATTERNS.some((pattern) => pattern.test(host))) return false;
    if (BRIEF_ALLOW_HOST_PATTERNS.some((pattern) => pattern.test(host))) return true;
  }
  ...
  return hasJapaneseRate(`${item?.title ?? ""} ${item?.summary ?? ""} ${item?.briefSummary ?? ""}`);
}

function briefSourceName(item) {
  return [
    item?.sourceName,
    item?.source,
...
function briefSourceHost(item) {
  const candidates = [
    item?.primaryLink?.url,
    ...(item?.sourceSignals ?? []).map((signal) => signal?.url),
    ...(item?.searchLinks ?? []).map((link) => link?.url),
  ];
...
function hasJapaneseRate(value) {
