...
export async function collectTrendTopics({ fetchImpl = fetch, now = new Date() } = {}) {
  ...
  const results = await Promise.allSettled(
    RSS_FEEDS.map((feed) => fetchRssEntries(fetchImpl, feed)),
  );
  const groups = [];
...
function logFeedDiagnostics(feedDiagnostics = [], totalEntries = 0) {
  ...
  for (const feed of okFeeds) {
    console.log(`[trend-aggregator] ok ${feed.id} count=${feed.count}`);
  }
  for (const feed of failedFeeds) {
    console.warn(`[trend-aggregator] fail ${feed.id} ${feed.error}`);
  }
}
...
function createFeedAbortSignal() {
