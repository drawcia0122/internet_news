...
function inferCategoriesForCluster(signals = []) {
  ...
  for (const signal of signals) {
    ...
    if (signal?.categoryHint) {
      let hintBoost = 2;
      if (signal.categoryHint === "matome" && isMatomeContext(`${signal.title ?? ""} ${signal.description ?? ""} ${signal.sourceName ?? ""}`)) {
        hintBoost = 5;
      }
      scores.set(signal.categoryHint, (scores.get(signal.categoryHint) ?? 0) + hintBoost);
    }
  }
...
function nichePenalty(categories, value, signals) {
  ...
  if (signals.length === 1 && signals.every((signal) => String(signal.source ?? "").toLowerCase() === "google news")) penalty += 4;
