...
export function normalizeAdultTrendItem(item, fetchedAt = new Date().toISOString()) {
  ...
  const historyKey = buildAdultHistoryKey({ item, source, sourceKey, sourceUrl, title });
  ...
  const rankingType = normalizeRankingType(item, trendType, sourceKey);
  ...
  return {
    ...
    ranking,
    rankingType,
    previousRank: normalizeNullableNumber(item.previousRank),
    rankDelta: normalizeNullableNumber(item.rankDelta) ?? normalizeNullableNumber(item.rankChange) ?? 0,
    adultTrendScore: normalizeNullableNumber(item.adultTrendScore) ?? 0,
    valueScore,
...
function buildAdultHistoryKey({ item, source, sourceKey, sourceUrl, title }) {
  const productCode = extractAdultProductCode(sourceUrl)
    || extractAdultProductCode(item?.id)
    || extractAdultProductCode(item?.title)
    || extractAdultProductCode(item?.sourceUrl);
  if (productCode) return `${normalizeSlug(source || sourceKey || "source")}-${productCode}`;
  const fallbackValue = sourceUrl || item?.id || title || item?.title || "";
  return `${normalizeSlug(sourceKey || source || "source")}-${normalizeSlug(fallbackValue).slice(0, 80)}`;
}

function normalizeRankingType(item, trendType, sourceKey) {
  const explicit = normalizeText(item?.rankingType ?? item?.rankingKind ?? "");
  if (explicit) return explicit;
  if (String(trendType ?? "").toLowerCase() === "new") return "new";
  if (String(trendType ?? "").toLowerCase() === "campaign") return "campaign";
  if (String(trendType ?? "").toLowerCase() === "sale") return "sale";
  const key = String(sourceKey ?? "").toLowerCase();
  if (key.includes("ranking") && key.includes("day")) return "daily";
  if (key.includes("ranking")) return "ranking";
  if (key.includes("new")) return "new";
  return String(trendType ?? "trend").toLowerCase();
}
...
function buildSummary({ title, source, categories, maker, rank, hotReasons }) {
  ...
  if (categories.includes("sale")) return `${target}はセール・キャンペーン対象です。割引率や還元、終了時刻の確認向きです。`;
  if (categories.includes("voice")) return `${target}は同人音声の注目枠です。ASMRやシチュエーション系の動きを見る入口になります。`;
  if (categories.includes("ai")) return `${target}はAI作品カテゴリで動きがあります。関連ジャンルの売れ筋確認に向いています。`;
  if (rank) return "";
  if (hotReasons[0]) return `${target}は${hotReasons[0]}として拾われています。`;
  return "";
}
...
function buildAdultFeature(featureKey, items, fetchedAt) {
  ...
  return {
    ...
    primaryGenre: first.adultPrimaryGenre ?? null,
    tags: labels,
...
function extractAdultProductCode(value) {
  const text = String(value ?? "");
  return text.match(/\b(RJ\d{6,8}|BJ\d{6,8}|VJ\d{6,8}|D[_-]?\d{6,8})\b/i)?.[1]?.replace(/-/g, "_").toUpperCase() ?? "";
}
