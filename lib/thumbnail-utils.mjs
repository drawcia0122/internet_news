...
function scoreEmbeddedImageCandidate(url) {
  ...
  try {
    ...
    if (/googleusercontent\.com$/.test(host) && !isProxyThumbnailUrl(url)) score += 20;
    ...
    if (isProxyThumbnailUrl(url)) score -= 220;
...
function extractDimensionHints(url) {
  return [
    ...String(url ?? "").matchAll(/[?&=_-]w(\d{1,4})(?:[&#/]|$)/gi),
    ...String(url ?? "").matchAll(/[?&=_-]h(\d{1,4})(?:[&#/]|$)/gi),
